...
let imgNotFoundHTML = '<img class="img-error" src="./images/404.png">';
...
function subscribeRequest(button, email, reductionAmount) {
	let inputUrl = document.getElementById("search").value;
	if (!inputUrl) return;
	let outputObj = {
		"email": email,
...
function putChartContainerContent(content){
	let chartContainer = document.getElementsByClassName("chart-container")[0];
	chartContainer.innerHTML = content;
	console.log(content);
	console.log(chartContainer.innerHTML);
}
function notFound(){
	putChartContainerContent(imgNotFoundHTML);
}
...
function showResult(){
	...
	fieldset.css('transform', 'translate(-50%, 0)');
	fieldset.animate({top: '16px'}, "slow", function() {
...
function serverRequest(startDate){
	let inputUrl = document.getElementById("search").value;
	if (!inputUrl) return;
	let outputObj = {url: inputUrl, startDate: startDate};
	$.ajax(
	{
...
function progressBar(){
	let bar = new RadialProgress(document.getElementById("bar"), {indeterminate:true,colorBg:"rgba(0,0,0,0)",colorFg:"silver",thick:5});
}
